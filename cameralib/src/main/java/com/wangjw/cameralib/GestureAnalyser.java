...
public class GestureAnalyser {
	...
	public GestureAnalyser(int swipeSlopeIntolerance,
						   int doubleTapMaxDelayMillis, int doubleTapMaxDownMillis) {
	...
	public GestureType getGesture(MotionEvent ev) {
		...
		for (int i = 0; i < numFingers; i++) {
			...
			averageDistance += Math.sqrt(Math.pow(finalX[i] - initialX[i], 2)
					+ Math.pow(finalY[i] - initialY[i], 2));
		}
	...
	public boolean isDoubleTap() {
		if (initialT - prevFinalT < doubleTapMaxDelayMillis
